...
import { mapLocation, mapRestaurant, type Row } from "./map";
import type { Restaurant, RestaurantLocation } from "../contract/models";

/**
 * Sanitises the PostgREST-style filter input and re-validates on the server:
 * `context.restaurantId` is what RLS uses, so a caller can never widen scope.
 */
...
export async function getRestaurantBySlug(slug: string, ctx: RequestContext = {}): Promise<Restaurant | null> {
  const row = await getDb().queryOne<Row>(ctx, `select ${RESTAURANT_COLUMNS} from restaurants where slug = $1`, [slug]);
  return row ? mapRestaurant(row) : null;
}

export async function getRestaurantById(id: string, ctx: RequestContext = {}): Promise<Restaurant | null> {
  const row = await getDb().queryOne<Row>(ctx, `select ${RESTAURANT_COLUMNS} from restaurants where id = $1`, [id]);
  return row ? mapRestaurant(row) : null;
}

export async function listPublicRestaurants(ctx: RequestContext = {}): Promise<Restaurant[]> {
...
export async function getLocationById(locationId: string, ctx: RequestContext = {}): Promise<RestaurantLocation | null> {
  const row = await getDb().queryOne<Row>(ctx, `select * from restaurant_locations where id = $1`, [locationId]);
  return row ? mapLocation(row) : null;
}
...
export async function createLocation(
  ...
  return db.write(ctx, async (tx) => {
    if (input.isPrimary) {
      await tx.query(`update restaurant_locations set is_primary = false where restaurant_id = $1`, [restaurantId]);
    }
    const row = await tx.queryOne<Row>(
      `insert into restaurant_locations
       ...
       returning *`,
      [
        restaurantId, input.name, input.slug ?? null, input.isPrimary ?? null, input.isActive ?? null,
        input.addressLine1 ?? null, input.addressLine2 ?? null, input.area ?? null, input.city ?? null,
        input.state ?? null, input.postalCode ?? null, input.country ?? null, input.phone ?? null,
        input.email ?? null, input.latitude ?? null, input.longitude ?? null,
        input.hours === undefined ? null : JSON.stringify(input.hours), input.sortOrder ?? null,
      ],
    );
...
export async function updateLocation(
  ...
  const db = getDb();
  return db.write(ctx, async (tx) => {
    const current = await tx.queryOne<Row>(`select restaurant_id from restaurant_locations where id = $1`, [locationId]);
    if (!current) throw new Error("Location not found");
    if (patch.isPrimary) {
      await tx.query(`update restaurant_locations set is_primary = false where restaurant_id = $1 and id <> $2`, [
    ...
    const row = await tx.queryOne<Row>(
      `update restaurant_locations set
       ...
       returning *`,
      [
        locationId, patch.name ?? null, patch.isPrimary ?? null, patch.isActive ?? null,
        patch.addressLine1 ?? null, patch.addressLine2 ?? null, patch.area ?? null, patch.city ?? null,
        patch.state ?? null, patch.postalCode ?? null, patch.country ?? null, patch.phone ?? null,
        patch.email ?? null, patch.latitude ?? null, patch.longitude ?? null,
        patch.hours === undefined ? null : JSON.stringify(patch.hours), patch.sortOrder ?? null,
      ],
    );
...
export async function deleteLocation(locationId: string, ctx: RequestContext): Promise<void> {
  const db = getDb();
  await db.write(ctx, async (tx: DbClient) => {
    await tx.query(`delete from restaurant_locations where id = $1`, [locationId]);
  });
}
