...
import { mapDelivery, mapDeliveryZone, num, str, type Row } from "./map";
import type { Delivery, DeliveryZone } from "../contract/models";
import type { DeliveryStatus } from "../contract/enums";
...
export async function getDeliveryZone(zoneId: string, ctx: RequestContext = {}): Promise<DeliveryZone | null> {
  const row = await getDb().queryOne<Row>(ctx, `select * from delivery_zones where id = $1`, [zoneId]);
...
export async function getDeliveryByOrder(orderId: string, ctx: RequestContext = {}): Promise<Delivery | null> {
  const row = await getDb().queryOne<Row>(ctx, `select * from deliveries where order_id = $1`, [orderId]);
