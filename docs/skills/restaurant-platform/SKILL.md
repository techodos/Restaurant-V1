...
- Migrations: `0013_notifications.sql` (outbox + push tokens + trigger), `0014_order_realtime.sql` (superseded), `0015_order_change_notify.sql` (NOTIFY trigger, drops the 0014 anon policy), `0016_rename_locations_table.sql` (see below), `0017`/`0018`/`0019` (`app_service`/`app_runtime` access to hosted Supabase's `auth` schema, see below), `0021_restore_team_members_auth_users_fk.sql`, `0022_restore_remaining_auth_users_fks.sql` (see the concurrent-edit incident below). Apply with `npm run db:migrate` (owner connection `DATABASE_URL_MIGRATOR`).
