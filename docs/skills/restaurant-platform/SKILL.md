...
Contents: 1 Overview · 2 Architecture · 3 Where new code goes · 4 Symbol map · 5 Rules easy to break · 6 Database and environment · 7 Notifications · 8 Live order tracking (SSE) · 9 Run on localhost · 10 Deploy (Vercel / other hosts) · 11 Troubleshooting · 12 Commands and tests · 13 Future: moving the backend out, one database per restaurant · 14 Known gaps · 15 Storefront cache (in-memory snapshot) · 16 Admin panel (`/admin`)
...
- Migrations: `0013_notifications.sql` (outbox + push tokens + trigger), `0014_order_realtime.sql` (superseded), `0015_order_change_notify.sql` (NOTIFY trigger, drops the 0014 anon policy), `0016_rename_locations_table.sql` (see below), `0017`/`0018`/`0019` (`app_service`/`app_runtime` access to hosted Supabase's `auth` schema, see below). Apply with `npm run db:migrate` (owner connection `DATABASE_URL_MIGRATOR`).
...
- `auth-service` queries `auth.users` directly (Supabase-shaped table); `repositories/team.ts` hashes passwords. On hosted Supabase this table is the real protected `auth` schema, not this app's shim — writes to it fail (section 6). `createCustomerAccount` (signup) and `repositories/team.ts` `createTeamMember`/password-set (insert/update `auth.users`) are broken there; only reads and `signInStaff` (fixed) work today.
...
- Admin UI (`/admin`, section 16) covers everything except staff management (deliberately excluded so far) and website/CMS — statuses/rows for those still change via SQL, so delivery for anything not yet in `/admin` needs the dispatcher script (dev) or a scheduler (prod).
